...
export default class OfflineManager {
    ...
    saveScoreOffline(scoreData) {
        ...
        try {
            ...
            const message = (i18nStrings && i18nStrings[lang] && i18nStrings[lang].offlineScoreSaved)
                ? i18nStrings[lang].offlineScoreSaved
            ...
            const lang = window.currentLang || 'zh-TW';
            const message = (i18nStrings && i18nStrings[lang] && i18nStrings[lang].offlineScoreError)
                ? i18nStrings[lang].offlineScoreError
    ...
    async syncOfflineScores() {
        ...
        if (failedUploads.length > 0) {
            ...
            const message = (i18nStrings && i18nStrings[lang] && i18nStrings[lang].syncError)
                ? i18nStrings[lang].syncError.replace('{count}', failedUploads.length)
            ...
            const message = (i18nStrings && i18nStrings[lang] && i18nStrings[lang].syncSuccess)
                ? i18nStrings[lang].syncSuccess
                : '所有離線遊戲紀錄皆已同步成功！';
            alert(message);
        }
    }
        ...
        if (retryButton) {
            retryButton.addEventListener('click', () => {
                ...
                if (navigator.onLine) {
                    ...
                    const message = (i18nStrings && i18nStrings[lang] && i18nStrings[lang].stillOffline)
                        ? i18nStrings[lang].stillOffline
                        : '網路尚未恢復連線，請稍後再試。';
                    alert(message);
                }
            });
