...
// 全域 UI
    ...
    function onAssetLoad() { /* ... (不變) ... */ assetsLoaded++; if (assetsLoaded === assetsToLoad) { console.log("所有圖片資源載入完成！"); player.image = player.defaultImage; player.loaded = true; } }
    
    // ✨ 步驟 3：載入所有圖片 (順序不變)

    // 玩家預設/勝利/失敗圖
    // // player.defaultImage.src = './images/xiao-yuan-bao.png'; ; // 舊的
    player.defaultImage.src = idleFrameSources[0]; // ✨ 讓預設圖 = 動畫第一幀
    player.defaultImage.onload = onAssetLoad;
    player.winImage.src = './images/xiao-yuan-bao-win.png'; player.winImage.onload = onAssetLoad;
    player.loseImage.src = './images/xiao-yuan-bao-lose.png'; player.loseImage.onload = onAssetLoad;
    
    // 掉落物圖
    itemTypes.forEach(type => { const img = new Image(); img.src = type.src; img.onload = onAssetLoad; itemImages[type.id] = img; });

    // ✨ 步驟 4：載入所有待機動畫幀 (idleFrames)
    idleFrameSources.forEach(src => {
        const img = new Image();
        img.src = src;
        img.onload = onAssetLoad; 
        player.idleFrames.push(img); // 存入 player 物件
    });
